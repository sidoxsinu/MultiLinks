...
// Config parameters corresponding to requested props
const color = '#ff6b35';
const speed = 0.6;
...
const initPlasma = () => {
  const containerEl = document.getElementById('plasma-bg');
  ...
  try {
    ...
    console.error("WebGL 2 not supported by browser:", err);
    return;
  }
  ...
  const loop = t => {
    if (contextLost || !isVisible || !isTabVisible) return;
  ...
  const io = new IntersectionObserver(([entry]) => {
    ...
    if (isVisible && !wasVisible && !contextLost && isTabVisible) {
      cancelAnimationFrame(raf);
      raf = requestAnimationFrame(loop);
    }
  }, { threshold: 0 });
  ...
  const handleVisibilityChange = () => {
    ...
    if (isTabVisible && !wasTabVisible && isVisible && !contextLost) {
      cancelAnimationFrame(raf);
      raf = requestAnimationFrame(loop);
